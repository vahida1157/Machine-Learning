...
data_raw = pd.read_csv('stockData.csv', index_col='Date')
...
param_grid_learned = {'kernel': ['linear'], 'C': [500], 'epsilon': [0.0001]}

data_train_for_Svr = data.loc['12/31/2019':'1/2/1992']
X_scaled_train_for_Svr = scaler.fit_transform(data_train_for_Svr.drop('close', axis=1).values)
y_train_for_Svr = data_train_for_Svr['close'].values

svr = SVR()
grid_search = GridSearchCV(svr, param_grid_learned, cv=2, n_jobs=-1, scoring='r2')
grid_search.fit(X_scaled_train_for_Svr, y_train_for_Svr)
